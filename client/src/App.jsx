...
const SUPABASE_ANON_KEY = "sb_publishable_U0lXP0cXCouIlW3av7UxAA_fpSAecBV";
...
export default function GrainDashboard() {
  ...
  const fetchPrices = useCallback(async () => {
    try {
      ...
      // Find the most recent scrape timestamp and filter to only those rows
      const mostRecent = rows.reduce((max, row) => {
        return row.scraped_at > max ? row.scraped_at : max;
      }, '');
      const cutoff = mostRecent ? new Date(new Date(mostRecent) - 2 * 60 * 60 * 1000).toISOString() : '';
      const recentRows = cutoff ? rows.filter(row => row.scraped_at >= cutoff) : rows;
      // deduplicate: latest per scraper+grain+futures_month+location
      const seen = new Set();
      const latest = recentRows.filter(row => {
